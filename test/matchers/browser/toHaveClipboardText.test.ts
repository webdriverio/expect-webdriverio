...
Received      : "actual text"`
    ...
    describe('Multi-Remote Browser', () => {
        ...
        test('success', async () => {
            ...
            const result = await thisContext.toHaveClipboardText(multiRemoteBrowser, 'some ClipBoard text', { ignoreCase: true, beforeAssertion, afterAssertion })
            expect(result.pass).toBe(true)
            expect(beforeAssertion).toHaveBeenCalledWith({
                matcherName: 'toHaveClipboardText',
                expectedValue: 'some ClipBoard text',
        ...
        test('failure check with message', async () => {
            ...
            expect(result.pass).toBe(false)
            expect(stripAnsi(result.message())).toEqual(`\
Expect multi-remote<chrome, firefox> to have clipboard text
