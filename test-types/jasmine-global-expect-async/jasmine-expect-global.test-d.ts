...
describe('Jasmine global type augmentations under `wdio/jasmine-framework`', () => {
    ...
    describe('Jasmine only cases', () => {
        it('should support expect correctly for non wdio types', async () => {
            ...
            expectTypeOf(expect(Promise.resolve('test')).toBeResolvedTo('plain string')).toMatchTypeOf<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve(42)).toBeResolvedTo(42)).toMatchTypeOf<PromiseLike<void>>()
            ...
            expectTypeOf(expect(Promise.reject('fail')).toBeRejectedWith('fail')).toMatchTypeOf<PromiseLike<void>>()
