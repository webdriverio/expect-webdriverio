...
describe('Jasmine global type augmentations under `wdio/jasmine-framework`', () => {
        ...
        describe('element or elements', () => {
            ...
            describe('toHaveText', () => {
                it('should return Promise<void>', async () => {
                    ...
                    expectTypeOf(expect(elementArray).toHaveText(wdioExpect.arrayContaining(['text1', 'text2']))).toEqualTypeOf<Promise<void>>()
                    expectTypeOf(expect(elementArray).toHaveText(jasmine.arrayContaining(['text1', 'text2']))).toEqualTypeOf<Promise<void>>()
                    expectTypeOf(expect(elementArray).not.toHaveText(jasmine.arrayContaining(['text1']))).toEqualTypeOf<Promise<void>>()
                    ...
                    expectTypeOf(expect(chainableArray).toHaveText(wdioExpect.arrayContaining([wdioExpect.stringContaining('text1')]))).toEqualTypeOf<Promise<void>>()
                    expectTypeOf(expect(chainableArray).toHaveText(jasmine.arrayContaining([jasmine.stringContaining('text1')]))).toEqualTypeOf<Promise<void>>()
    ...
    describe('Jasmine only cases', () => {
        it('should support expect correctly for non wdio types', async () => {
            expectTypeOf(expect(Promise.resolve('test')).toBeResolved()).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).toBeResolvedTo(wdioExpect.stringContaining('test error'))).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).toBeResolvedTo(jasmine.stringContaining('test error'))).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).toBeResolvedTo('plain string')).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve(42)).toBeResolvedTo(42)).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).not.toBeResolvedTo(wdioExpect.not.stringContaining('test error'))).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).not.toBeResolvedTo(jasmine.stringContaining('test error'))).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.reject('fail')).toBeRejectedWith('fail')).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).toBeRejected()).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).not.toBeResolved()).toExtend<PromiseLike<void>>()
            expectTypeOf(expect(Promise.resolve('test')).not.toBeRejected()).toExtend<PromiseLike<void>>()
